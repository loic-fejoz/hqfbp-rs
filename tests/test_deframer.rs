...
fn test_deframer_chunked() {
    let mut deframer = Deframer::new();
    let mut generator =
        PDUGenerator::new(Some("F4JXQ-1".to_string()), None, Some(10), None, None, 1);
...
fn test_deframer_announcement_and_crc() {
    ...
    let mut generator = PDUGenerator::new(
        ...
        None,
        Some(vec![
            hqfbp_rs::ContentEncoding::H,
            hqfbp_rs::ContentEncoding::Crc32,
        ]),
...
fn test_deframer_multi_sender_interleaved_announcements() {
    ...
    for i in range(0..max_len) {
        if i < pdus1.len() {
            all_pdus.push(&pdus1[i]);
        }
        if i < pdus2.len() {
            all_pdus.push(&pdus2[i]);
        }
        if i < pdus3.len() {
            all_pdus.push(&pdus3[i]);
        }
    }
...
fn range(r: std::ops::Range<usize>) -> std::ops::Range<usize> {
    r
}
