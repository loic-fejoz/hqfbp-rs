...
fn test_mandatory_msg_id() {
    ...
    assert!(res.is_err());
    assert!(res.unwrap_err().to_string().contains("Message-Id is mandatory"));
}
...
fn test_human_readable() {
    ...
    assert_eq!(readable.get("Src-Callsign").unwrap().as_str(), Some("FOSM-1"));
    assert_eq!(readable.get("Content-Type").unwrap().as_str(), Some("image/png"));
    
    let ce = readable.get("Content-Encoding").unwrap().as_array().unwrap();
