...
fn test_regression_repeat_rs_pdu_level() {
    ...
    assert_eq!(messages.len(), 1, "Should have recovered the message from a single repeated PDU");
...
fn test_regression_systematic_rs_header_corrupt() {
    ...
    let mut ann_header = hqfbp_rs::Header::default();
    ann_header.message_id = Some(1);
    ann_header.content_encoding = Some(hqfbp_rs::EncodingList(vec![
        ContentEncoding::H,
        ContentEncoding::ReedSolomon(255, 223),
    ]));
    ann_header.set_media_type(Some(hqfbp_rs::MediaType::Type("application/vnd.hqfbp+cbor".to_string())));
    
    let ann_body = minicbor::to_vec(&ann_header).unwrap();
    let ann_pdus = ann_gen.generate(&ann_body, Some(hqfbp_rs::MediaType::Type("application/vnd.hqfbp+cbor".to_string()))).unwrap();
