...
mod tests {
    ...
    fn test_rq_rs_user_params() {
        ...
        let mut generator = PDUGenerator::new(
            ...
            None,
            Some(encs),
            Some(vec![ContentEncoding::H, ContentEncoding::Crc32, ContentEncoding::Repeat(50)]),
            1,
        );
                    ...
                    Event::PDU(pe) => {
                        ...
                        if pe.payload != clean_unpack.1 {
                            ...
                            if first_mismatch.is_none() {
                                first_mismatch = Some((idx, clean_unpack.1.to_vec(), pe.payload.to_vec()));
                            }
                        }
                    }
                    Event::Message(me) => {
        ...
        let msg = format!("Received: {}, Accepted: {}, Mismatched: {}, Total: {}, Recovered: {} Len: {} Match: {}\n", 
            count_received, count_accepted, mismatch_count, pdus.len(), recovered_data.is_some(), 
            recovered_data.as_ref().map(|r| r.len()).unwrap_or(0),
            if let Some(ref rd) = recovered_data { rd == &data } else { false });
        ...
        if let Some(ref rd) = recovered_data {
            if rd != &data {
                ...
                if let Some((idx, clean, faulty)) = first_mismatch {
                    mismatch_dump = format!("FIRST MISMATCH AT PDU {}:\n  CLEAN: {:02x?}\n  FAULTY: {:02x?}\n", 
                        idx, &clean[..32.min(clean.len())], &faulty[..32.min(faulty.len())]);
                }
                let diff_msg = format!("DIFFS: {:?}\n{}", diffs, mismatch_dump);
